...
if numOpts < 2:
	...
	if not os.path.exists('prank'):os.mkdir('prank')
	os.chdir('prank')
	sh = open('prank.sh','w')
	aaseqs = os.listdir(options.path)
	for fas in aaseqs:
		if re.search('fasta$',fas):
	...
	os.chdir('../')
	if not os.path.exists('gblock'):os.mkdir('gblock')
	os.chdir('gblock')
	prankseqs = os.listdir(prank_path)
	for fas in prankseqs:
		if re.search('best.fas$',fas):
			basename = (fas.split('.'))[0]
			fasfile = '%s/%s' %(prank_path,fas)
			os.system('Gblocks %s -e=.fa -b5=N -t=p -b0=10' %fasfile)
			#print('Gblocks %s -e=.fa -b5=N -t=p -b0=10' %fasfile)
			
	os.chdir('../')
	if not os.path.exists('convert_paml'):os.mkdir('convert_paml')
	os.chdir('convert_paml')
	sh = open('convert.sh','w')
	aaseqs = os.listdir(prank_path)
	for fas in aaseqs:
		if re.search('fa$',fas):
			basename = (fas.split('.'))[0]
			fasfile = '%s/%s' %(prank_path,fas)
